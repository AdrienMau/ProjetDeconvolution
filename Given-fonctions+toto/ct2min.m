function OutVec=ct2min(filename, timing, cutoffs, mode, savefile, InVec)
...
if nargin < 3
   cutoffs = [1/3 1000 20 0];
	disp('Set default cutoff values 1/3 relErr, 0 cnts min 1000 cntsMax and maximum trace=20');
end
if nargin < 2
 	timing=[1 100];
...
pkind = find(pkdata(:,10)<(pkdata(:,5)*cutoffs(1)) & pkdata(:,5)> 0 & pkdata(:,5)< cutoffs(2) & pkdata(:,5)> cutoffs(4)) ;
...
if size(ninddata,1)>0
   [nmsddata, fmsddata] = ct2_msd(ntrcdata);	% calculate mean square displacement
...
if mode==1
  ...
  [x,p] = pdf_old (I,dI,0,MaxIntensity*2);
...
if ~isempty(NoTrace)
 ...
 % if you want to save the bleaching curve...engage this command line
 ble=HistTrcLen';
    save([filename,'.bleach.dat'],'ble','-ascii');
