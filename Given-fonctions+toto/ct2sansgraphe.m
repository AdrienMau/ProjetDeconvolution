function OutVec=checkthattoo(filename, timing, cutoffs, mode, savefile, InVec)
% function OutVec = CheckThatToo (filename, timing, cutoffs, mode, savefile, InVec)
% --
...
%
% InVec and OutVec are not yet implemented
% -- 
...
ninddata=inddata(trclen<cutoffs(3),:);
indpk=inddata(trclen>=cutoffs(3),2:end);
...
if size(ninddata,1)>0
   [nmsddata, fmsddata] = ct2_msd(ntrcdata);	% calculate mean square displacement% Modifié le 010608
...
WidthMax = x(find(p==max(p)));
        %clf, subplot(321)% Modifié le 010608
%plot (x,p)
...
[x,p] = pdf_old (Dark,dDark);
        %subplot(321), plot (x,p)% Modifié le 010608
z = [];
x1 = x';
  p1 = p';
  z = [x1,p1];
%if you want to save the background pdf, engage this command:
save([filename,'.backpdf.dat'],'z','-ascii');% Modifié le 010608
  
%end
        %DarkMax = x(find(p==max(p)));% Modifié le 010608
        %xlabel('dark-rate (cnt)'), ylabel('\rho(d) (1/cnt)')% Modifié le 010608
        %title(filename,'Interpreter','none')% Modifié le 010608
        %text(0.1,0.9,sprintf('d_{max}=%4.2f',DarkMax),'Units','normalized')% Modifié le 010608
...
if mode==1
  ...
  p = hist(I,x);
subplot(3,2,2), stairs(x,p);% Modifié le 010608
else
  [x,p] = pdf_old (I,dI,0,MaxIntensity);
subplot(3,2,2), plot (x,p)% Modifié le 010608
  z = [];
  x1 = x';
  p1 = p';
  z = [x1,p1];
...
IntensityMax = x(find(p==max(p)));%Modifié le 010608
%xlabel('peak-intensity (cnt)'), ylabel('\rho(I_{p}) (1/cnt)')%Modifié le 010608

text(0.5,0.9,['I_{max} = ',num2str(IntensityMax)],'Units','normalized');%Modifié le 010608
%text(0.5,0.8,['<I>  = ',num2str(IntensityMean)],'Units','normalized');%Modifié le 010608
%text(0.5,0.7,['I_{dark}= ',num2str(DarkMean)],'Units','normalized');%Modifié le 010608
%text(0.5,0.6,['N= ',num2str(size(npkdata,1)),' (',num2str(100*size(npkdata,1)/opklen),'%)'],'Units','normalized');%Modifié le 010608
%text(0.5,0.5,['t_{ill}= ',num2str(till),' ms'],'Units','normalized');%Modifié le 010608

%************************************position des pics détectés et suivis***********************************
if ~isempty(ntrcdata)
...
%********************************************plot histogram of trace-lengths and calulation of bleaching time% Modifié le 010608***************
%subplot(3,2,4)
...
[N,X]=hist (TrcLen,MaxTrc-1);% Modifié le 010608
z = [];% Modifié le 010608
X1 = X';% Modifié le 010608
N1 = N';% Modifié le 010608
z = [X1,N1];% Modifié le 010608
save([filename,'.TRCpdf.dat'],'z','-ascii');% Modifié le 010608
%	stairs (1:MaxTrc+2,[0 HistTrcLen 0 0]);% Modifié le 010608
%	xlabel ('length of trace (pxl)'), ylabel ('occurrence');% Modifié le 010608
%   text(0.5,0.9,sprintf(' N_{datapoints}=%4d',size(ntrcdata,1)),'Units','normalized');% Modifié le 010608
	%txt = axis; text(0.65*txt(2),0.8*txt(4),sprintf('N1=%3d',FirstPeaksFound))
	%changed by GAB & PL
%	text(0.65,0.8,['N_{traces}=',num2str(size(ninddata,1))],'Units','normalized');% Modifié le 010608
%	text(0.65,0.2,sprintf('t_{B}=%4.2f',tBleach),'Units','normalized');% Modifié le 010608
%	text(0.65,0.1,sprintf('+- %4.2f',std_tBleach),'Units','normalized');% Modifié le 010608
   ...
   if mode==1
     ...
     subplot(3,2,5), stairs(x,p);
      ...
      if mode == 2
        subplot(3,2,5);
         hold on;
         fmsddata=fmsddata*4e-2; % go from pxl => um^2
...
%           	  filename = '';
					filename = sprintf('dist%02.0f.mat', step);
             	doit = ['save ', filename, ' OnzeData -ascii'];
               eval(doit);
               % YUK !!!!! who the hell programmed this?
               % save(filename,'OnzeData','-ascii') 
               % 
...
%         hold off%corrigé le 010608
      else
   		[x,p] = pdf_old (Diff,dDiff,0,4*MeanD);
%	   	subplot(3,2,5), plot (x,p)% Modifié le 010608
%			xlabel('diffusion constant (pxl/lag)'), ylabel('\rho(D) (lag/pxl)')% Modifié le 010608
%	     	text(0.5,0.8,sprintf(' <D>=%4.2f +/- %4.2f',MeanD,std(Diff)),'Units','normalized')% Modifié le 010608
      end % if mode
	end % if length(ind)
%	xlabel('diffusion constant (pxl/lag)'), ylabel('\rho(D) (lag/pxl)')
...
%*********************************************linear MSD-plot************************************************
%	subplot(3,2,6)% Modifié le 010608
...
%   plot(1:ntrc,tdat(:,1),'b*',1:ntrc,tdat(:,1)+tdat(:,2),'b^',1:ntrc,tdat(:,1)-tdat(:,2),'bv');% Modifié le 010608
...
%	hold off% Modifié le 010608
%	axis ('normal')% Modifié le 010608
%	xlabel ('time lag'), ylabel('MSD (pixel^2)')% Modifié le 010608
%	txt = axis; text(0.65*txt(2),0.2*txt(4),sprintf('#trc=%3d',size(ninddata,1)))

else
%  	text(0.25,0.9,'No Traces.','Units','normalized');% Modifié le 010608
end % if ~isempty(Notrace)

%pause(1)%corrigé le 010608
%---------------------------------------------------------
%report result
%set(gcf,'PaperType','A4','PaperOrientation','Portrait','PaperUnits','normalized','PaperPosition',[0.05 0.05 0.95 0.95]);%corrigéle01068
