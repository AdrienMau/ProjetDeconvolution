...
for iX=1:nX
    for iP=1:nP
        for iY=1:nY
            ...
            %icorr = image;
            icorr = SubImage - gaussian(aback,xxsize,yysize);
            ...
            while MaxCorr>noise*threshold & peak<MaxNoPk
                ...
                Ltrc=size(trace,1);
                [iY, p(1)+xfits-1, yfits-1+p(2)];
                for iTrc=1:Ltrc
                    [trace(iTrc,1), iY];
                    if ((trace(iTrc,1)==iY) & (abs(p(1)+xfits-1-trace(iTrc,2))<=0.1) & (abs(yfits-1+p(2)-trace(iTrc,3))<=0.1))
                        [trace(iTrc,1), p(1)+xfits-1, yfits-1+p(2)]
