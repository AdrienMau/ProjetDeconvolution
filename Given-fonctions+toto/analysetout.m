function OutVec = analysetout (file, NSeq, mode, Conf, InVec)
%--------------------------------------------------------
% ANALYZE.M
%
...
% modif le 19.02.2001 lc de analyze
%--------------------------------------------------------
...
%%%%%%%%%%% modif par rapport à analyze LC 2001/02/19%%%%%%%%%%%
...
for k=1:length(files)
   ...
   if SPot>0
      Strcdata(:,1)=Strcdata(:,1)+maxim;		% new imagenumber
   end
   disp(['*  ',num2str(length(Strcdata)),' traces in file ',files(k).name,sprintf(' (%d/%d)',k,length(files))]);
   trcdata=[trcdata; Strcdata];
   
   if ~isempty(trcdata)
      maxim=max(trcdata(:,1))+20;
   end
end
...
for k=1:length(files)
   ...
   if SPom>0
      Smsddata(:,1)=Smsddata(:,1)+maxim;		% new imagenumber
   end
   disp(['*  ',num2str(length(Smsddata)),' msd in file ',files(k).name,sprintf(' (%d/%d)',k,length(files))]);
   msddata=[msddata; Smsddata];
   
   if ~isempty(msddata)
      maxim=max(msddata(:,1))+20;
   end
end
